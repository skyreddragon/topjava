...
//    }
    ...
    public static List<UserMealWithExceed> getExceeded(Map<Long, UserMeal> mealList, int caloriesPerDay) {
        List<UserMealWithExceed> result = new ArrayList<>();
        Map<LocalDate, Integer> caloriesSumByDate = mealList.values().stream()
                .collect(
                        Collectors.groupingBy(um -> um.getDateTime().toLocalDate(),
                                Collectors.summingInt(UserMeal::getCalories))
                );
        mealList.entrySet().stream()
                .forEach(entry -> result.add(createWithExceed(entry.getValue(), caloriesSumByDate.get(entry.getValue().getDateTime().toLocalDate()) > caloriesPerDay)));
        return result;
    }
